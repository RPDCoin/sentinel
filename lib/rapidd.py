...
class rapidDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some rapidd methods
    ...
    @classmethod
    def from_rapid_conf(self, rapid_dot_conf):
        from rapid_config import rapidConfig
        config_text = rapidConfig.slurp_config_file(rapid_dot_conf)
        creds = rapidConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_current_masternode_vin(self):
        from rapidlib import parse_masternode_status_vin
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = rapidlib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = rapidlib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = rapidlib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return (getinfo['protocolversion'] >= config.min_rapidd_proto_version_with_sentinel_ping)
