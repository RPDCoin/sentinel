...
sentinel_cfg = rapidConfig.tokenize(sentinel_config_file)
sentinel_version = "1.1.0"
min_rapidd_proto_version_with_sentinel_ping = 70208


def get_rapid_conf():
    home = os.environ.get('HOME')

    rapid_conf = os.path.join(home, ".rapidcore/rapid.conf")
    if sys.platform == 'darwin':
        rapid_conf = os.path.join(home, "Library/Application Support/rapidCore/rapid.conf")

    rapid_conf = sentinel_cfg.get('rapid_conf', rapid_conf)
