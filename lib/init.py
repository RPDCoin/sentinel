...
def has_rapid_conf():
    ...
    valid_rapid_conf = False

    # ensure rapid_conf exists & readable
    #
    # if not, print a message stating that rapid Core must be installed and
    # configured, including JSONRPC access in rapid.conf
    try:
        f = io.open(config.rapid_conf)
        valid_rapid_conf = True
    except IOError as e:
        print(e)

    return valid_rapid_conf
...
def main():
    ...
    if not has_rapid_conf():
        print("rapidCore must be installed and configured, including JSONRPC access in rapid.conf")
        sys.exit(1)
