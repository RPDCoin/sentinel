...
class GovernanceObject(BaseModel):
    ...
    # sync rapidd gobject list with our local relational DB backend
    @classmethod
    def sync(self, rapidd):
        golist = rapidd.rpc_command('gobject', 'list')
        ...
        try:
            ...
            for item in golist.values():
                (go, subobj) = self.import_gobject_from_rapidd(rapidd, item)
    ...
    @classmethod
    def import_gobject_from_rapidd(self, rapidd, rec):
        import decimal
        import rapidlib
        import inflection
        ...
        # shim/rapidd conversion
        object_hex = rapidlib.SHIM_deserialise_from_rapidd(object_hex)
        objects = rapidlib.deserialise(object_hex)
        subobj = None
        ...
        # exclude any invalid model data from rapidd...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from rapidd, with every run
        ...
        # get/create, then sync payment amounts, etc. from rapidd - rapidd is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(rapidd)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from rapidd! %s" % e)
            govobj.vote_delete(rapidd)
            return (govobj, None)
    ...
    def vote_delete(self, rapidd):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(rapidd, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, rapidd, signal, outcome):
        import rapidlib
        ...
        output = rapidd.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = rapidlib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(rapidd, signal)

    def sync_network_vote(self, rapidd, signal):
        printdbg('\tsyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = rapidd.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    govobj_type = rapidD_GOVOBJ_TYPES['proposal']
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 rapid addr, non-multisig
            if not rapidlib.is_valid_rapid_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid rapid address for network [%s], returning False" % (self.payment_address, config.network))
                return False
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (rapidlib.blocks_to_seconds(superblockcycle) / 2) +
    ...
    def is_deletable(self):
        ...
        # TBD (item moved to external storage/rapidDrive, etc.)
        return False
    ...
    def get_prepare_command(self):
        import rapidlib
        obj_data = rapidlib.SHIM_serialise_for_rapidd(self.serialise())
    ...
    def prepare(self, rapidd):
        try:
            object_hash = rapidd.rpc_command(*self.get_prepare_command())
...
class Superblock(BaseModel, GovernanceClass):
    ...
    govobj_type = rapidD_GOVOBJ_TYPES['superblock']
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not rapidlib.is_valid_rapid_address(addr, config.network):
    ...
    def hash(self):
        import rapidlib
        return rapidlib.hashit(self.serialise())
...
class Watchdog(BaseModel, GovernanceClass):
    ...
    govobj_type = rapidD_GOVOBJ_TYPES['watchdog']
    only_masternode_can_submit = True

    @classmethod
    def active(self, rapidd):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at >= (now - rapidd.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
        return resultset

    @classmethod
    def expired(self, rapidd):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at < (now - rapidd.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
    ...
    def is_expired(self, rapidd):
        now = int(time.time())
        return (self.created_at < (now - rapidd.SENTINEL_WATCHDOG_MAX_SECONDS))

    def is_valid(self, rapidd):
        if self.is_expired(rapidd):
            return False

        return True

    def is_deletable(self, rapidd):
        if self.is_expired(rapidd):
            return True
