...
def is_valid_rapid_address(address, network='mainnet'):
    ...
    rapid_version = 140 if network == 'testnet' else 76
    ...
    try:
        ...
        # rescue from exception, not a valid rapid address
        return False

    if (address_version != rapid_version):
        return False
...
# shims 'til we can fix the rapidd side
def SHIM_serialise_for_rapidd(sentinel_hex):
    from models import rapidD_GOVOBJ_TYPES
    # unpack
    ...
    # add 'type' attribute
    obj[1]['type'] = rapidD_GOVOBJ_TYPES[govtype]

    # superblock => "trigger" in rapidd
    if govtype == 'superblock':
        obj[0] = 'trigger'

    # rapidd expects an array (even though there is only a 1:1 relationship between govobj->class)
    obj = [obj]

    # re-pack
    rapidd_hex = serialise(obj)
    return rapidd_hex


# shims 'til we can fix the rapidd side
def SHIM_deserialise_from_rapidd(rapidd_hex):
    from models import rapidD_GOVOBJ_TYPES

    # unpack
    obj = deserialise(rapidd_hex)
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('rapid.conf')
